...
{	
	...
	{
		dup2_double(ft_lst_avant_dernier_last(pipex.cmd)->pipe[0],pipex.outfile);
	}
	else if(i > 2 && i < pipex.n_cmd-1)
	{
		dup2_double(ft_lst_avant_dernier_last(pipex.cmd)->pipe[0],ft_lstlast(pipex.cmd)->pipe[1]);
